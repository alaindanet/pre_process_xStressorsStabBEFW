...
tar_map(
  ...
  mutate(
    file_no_ts = map(full_path, ~list.files(.x) %>% .[!str_detect(., "_ts")]),
    file_ts = map(full_path, ~list.files(.x) %>% .[!str_detect(., "_ts")]),
  )
  ),
tar_target(file_no_ts, list.files(full_path) %>% .[!str_detect(., "_ts")],)
)
