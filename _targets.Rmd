...
list(
  ...
  sim_ts_mat_d %>%
        ...
        mutate(
          species = map(species,
            function (x) {
            matrix(x, nrow = 500)[300:500, , drop = FALSE]
            }
          ),
          stoch = map(stoch,
            function (x) {
...
```{r, eval = FALSE}
tar_load(c(sim_dir_d, sim_files_ts_d))
ti
tu <- matrix(ti$stoch[[1000]], nrow = 500)
head(tu)
```
...
tar_load(sim_stab_d)
tar_load(sim_prep_d)
sp <- matrix(1:4, nrow = 2)
stoch <- matrix(rep(c(.5, 1), each = 2), nrow = 2)
sp * stoch
```
